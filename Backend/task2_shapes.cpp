...
// #endif
// // canny done till now ...
// // lines circles elipices not till now
...
//             val +=
//                 dst.at<float>(y + ky, x + kx) * (kernel[ky + 1][kx + 1] /
//                 sum);
//           }
//         }
...
//   static void computeSobelGradients(const cv::Mat &blurred, cv::Mat
//   &magnitude,
//                                     cv::Mat &direction) {
...
//         // FIX: Using L1 norm to match OpenCV cv::Canny behavior
//         magnitude.at<float>(y, x) = std::abs(gx) + std::abs(gy);

//         // Direction in degrees [-180, 180]
//         float angle = std::atan2(gy, gx) * 180.0f / M_PI;
//         if (angle < 0)
//           angle += 180.0f; // Only need [0, 180]
...
//           // 45 degrees corresponds to top-left / bottom-right in image
//           // coordinates (y down)
...
//           // 135 degrees corresponds to top-right / bottom-left
//           q = magnitude.at<float>(y - 1, x + 1);
//           r = magnitude.at<float>(y + 1, x - 1);
//         }
...
//     // Step 1: Identify all strong edges
...
//     // Step 2: Hysteresis tracking via Depth First Search
...
//   static std::vector<Line> detectLines(const cv::Mat &edges, int threshold) {
...
//     std::vector<float> sins(num_theta);
//     std::vector<float> coss(num_theta);
...
//     // Extract peaks using sort and Non-Maximum Suppression (NMS)
...
//         // Suppress if the line is very similar to an already accepted
//         stronger
//         // line
//         if (d_rho < 20.0f && d_theta < 0.17f) { // ~10 degrees, 20 pixels
//           keep = false;
//           break;
//         }
//       }
...
//   static std::vector<Circle> detectCircles(const cv::Mat &edges, int minR,
//                                            int maxR, int threshold) {
//     std::vector<Circle> raw_circles;

//     for (int r = minR; r <= maxR; r += 2) {
//       std::vector<std::vector<int>> acc(edges.rows,
//                                         std::vector<int>(edges.cols, 0));
//       std::vector<std::pair<int, int>> circle_pts;

//       // Midpoint Circle Algorithm to generate unique perimeter points
//       int x_c = r, y_c = 0;
//       int err = 0;
//       while (x_c >= y_c) {
...
//       // Remove any sparse duplicates just in case
...
//         for (int x = 0; x < edges.cols; ++x) {
//           int v = acc[y][x];
//           // Increase structural minimum threshold to 65% to suppress natural
//           fractal textures/noise int min_v = std::max((int)threshold,
//           (int)(circle_pts.size() * 0.65f));

//           if (v >= min_v) {
//             // Plateau-aware local maximum check in a 3x3 neighborhood
//             bool is_max = true;
...
//                 {
//                   if (acc[ny][nx] > v || (acc[ny][nx] == v && (dy < 0 || (dy
//                   == 0 && dx < 0)))) {
//                     is_max = false;
//                     break;
//                   }
...
//     // Cross-radius NMS Clustering
//     std::vector<Circle> circles;
//     for (const auto &c : raw_circles) {
//       bool keep = true;
//       for (const auto &existing : circles) {
//         float dist = std::hypot(c.x - existing.x, c.y - existing.y);
//         // Suppress if circles share similar centers (within 20 pixels) to
//         // prevent concentric overlapping
//         if (dist < 20.0f) {
//           keep = false;
//           break;
//         }
...
//       if (keep) {
//         circles.push_back(c);
//         // Cap the maximum number of circles to prevent flooding the UI with
//         noise if (circles.size() > 50) break;
//       }
//     }
...
//     // Fix: Randomized RANSAC-style pair sampling per Kanatani et al.
//     srand(42);
//     int max_attempts = 15000;
//     int attempts = 0;
...
//       // 1D Accumulator over all edge points
//       for (int k = 0; k < N; k++) {
//         if (k == i || k == j)
//           continue;
//         const auto &p3 = edge_pts[k];

//         float d_p3_sq = (p3.x - x0) * (p3.x - x0) + (p3.y - y0) * (p3.y -
//         y0); float d_p3 = std::sqrt(d_p3_sq); if (d_p3 > a)
//           continue;
...
//         // Add epsilon to avoid floating point division zero-exceptions
//         float denom = std::abs(a * a - (d_p3_sq - d_focus * d_focus)) +
//         1e-4f; float b_sq = (a * a * d_focus * d_focus) / denom;
...
//       if (max_votes > 15 && best_b >= 5) {
//         raw_ellipses.push_back(
//             {(int)x0, (int)y0, (int)a, best_b, alpha, max_votes});
//       }
//     }

//     // NMS Clustering to eliminate the "Pink Mess" false positives
...
//         // Suppress if the center and shape are too similar to a stronger
//         // ellipse
//         if (dist < 20.0f && da < 15.0f && db < 15.0f) {
//           keep = false;
//           break;
//         }
//       }
...
// py::array_t<unsigned char> apply_canny_wrapper(py::array_t<unsigned char>
// img,
//                                                int threshold1, int
//                                                threshold2) {
...
//   int count = cv::countNonZero(edges);
//   std::cout << "[Backend] Canny Edge pixels detected: " << count <<
//   std::endl; return mat_to_numpy(edges);
// }

// py::array_t<unsigned char>
// detect_lines_wrapper(py::array_t<unsigned char> img_edges,
//                      py::array_t<unsigned char> img_orig, int line_thresh) {
...
//   auto lines = ShapeDetector::detectLines(edges, line_thresh);
//   std::cout << "[Backend] Lines detected: " << lines.size()
//             << " with threshold " << line_thresh << std::endl;
//   for (const auto &l : lines) {
//     float a = std::cos(l.theta), b = std::sin(l.theta);
//     float x0 = a * l.rho, y0 = b * l.rho;
//     cv::Point pt1(cvRound(x0 + 1000 * (-b)), cvRound(y0 + 1000 * (a)));
//     cv::Point pt2(cvRound(x0 - 1000 * (-b)), cvRound(y0 - 1000 * (a)));
//     cv::line(result, pt1, pt2, cv::Scalar(0, 0, 255), 2); // Red
//   }
...
//                        py::array_t<unsigned char> img_orig, int min_r,
//                        int max_r, int circle_thresh) {
//   auto edges = numpy_to_mat(img_edges);
//   auto orig = numpy_to_mat(img_orig);
//   cv::Mat result;
...
//   auto circles =
//       ShapeDetector::detectCircles(edges, min_r, max_r, circle_thresh);
//   std::cout << "[Backend] Circles detected: " << circles.size()
//             << " | min_R:" << min_r << " max_R:" << max_r
//             << " thresh:" << circle_thresh << std::endl;
//   for (const auto &c : circles) {
//     cv::circle(result, cv::Point(c.x, c.y), c.r, cv::Scalar(0, 255, 0),
//                2); // Green
//     cv::circle(result, cv::Point(c.x, c.y), 2, cv::Scalar(255, 0, 0),
//                3); // Center
//   }
//   return mat_to_numpy(result);
// }
...
//   std::cout << "[Backend] Ellipses detected (Xie & Ji alg): " <<
//   ellipses.size()
//             << std::endl;
//   for (const auto &e : ellipses) {
//     cv::ellipse(result, cv::Point(e.x0, e.y0), cv::Size(e.a, e.b),
//                 e.alpha * 180.0 / M_PI, 0, 360, cv::Scalar(255, 0, 255),
//                 2); // Magenta
//     cv::circle(result, cv::Point(e.x0, e.y0), 2, cv::Scalar(255, 0, 0),
//                3); // Center
//   }
//   return mat_to_numpy(result);
// }
...
//   m.doc() = "Task 2 Backend: Canny Edge & Hough Shape Detection from
//   scratch"; m.def("apply_custom_canny", &apply_canny_wrapper,
...
//         py::arg("edges"), py::arg("original"), py::arg("line_thresh"));

//   m.def("detect_circles", &detect_circles_wrapper,
//         "Detect & superimpose circles", py::arg("edges"),
//         py::arg("original"), py::arg("min_r"), py::arg("max_r"),
//         py::arg("circle_thresh"));
...
public:
  ...
  static std::vector<Circle> detectCircles(const cv::Mat &edges) {
    ...
    for (int r = minR; r <= maxR; r += 2) {
      ...
      // Small circles easily hit 35% by chance, so require a strict 65%.
      // Massive screen-sized circles can find false alignments in grass, so keep a solid 45% floor.
      float ratio = 0.65f - 0.20f * ((float)(r - minR) / (maxR - minR + 1));
